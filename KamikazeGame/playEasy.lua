...
if event.params==nil then
	...
	livesNum = display.newText(gameScreenGroup, "5", middleX-300, 5, "Verdana", 30)
...
function moveBoatSimEasy(event)
...
function addPlaneEasy(event)
	...
	gameScreenGroup:insert(planeFlying)
 	planeFlying.x = math.random(0,1200)
  	...
  	movePlaneEasy(planeFlying.x)
end	

function movePlaneEasy(compareX1)
...
--		print("this is pointRight")
	...
	planeFlying:play()
	planeMove = transition.to(planeFlying, {time = 1500, x = compareX2, y = display.contentHeight+10})
end

function tickingTimeEasy(event)
 theTime = theTime - 1
 timerNum.text = theTime 
end

function check4GameOverEasy()
	if lives<=0 then
--		print("gameOver")
		gameOverEasy("lives")	
	end
...
function gameOverEasy(event)
	print("gameOver on playEasy")
	if event == "lives" then
		lives=0
	end	
...
function removeObjectEasy(event)
...
function scene:enterScene(event)
	print("enterScene playEasy")
	storyboard.purgeAll()
--timers
	addPlaneTimer = timer.performWithDelay(950, addPlaneEasy, 31)
	endTimer = timer.performWithDelay(30000, gameOverEasy, 1)
	--timer for displayed timer
	countdownTimer=timer.performWithDelay(1000, tickingTimeEasy, 30)
...
-- Listeners
	destroyer:addEventListener("touch", moveBoatSimEasy)		
	Runtime:addEventListener("touch", spawnBallEasy)
	Runtime:addEventListener("collision", removeObjectEasy)
	Runtime:addEventListener("accelerometer", moveBoatEasy)
	musicIcon:addEventListener("touch", onPlayTouch)
	sfxIcon:addEventListener("touch", toggleSound)
end
...
-- Listeners		
	Runtime:removeEventListener("touch", spawnBallEasy)
	Runtime:removeEventListener("collision", removeObjectEasy)
	destroyer:removeEventListener("touch", moveBoatSimEasy)
	Runtime:removeEventListener("accelerometer", moveBoatEasy)
	musicIcon:removeEventListener("touch", onPlayTouch)
	sfxIcon:removeEventListener("touch", toggleSound)
end
