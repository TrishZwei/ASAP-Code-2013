...
print("I'm on the Medium screen")
if event.params==nil then
	...
	livesText = display.newText(gameScreenGroup, "Lives:", middleX-420, 5, "Verdana", 30)
	livesNum = display.newText(gameScreenGroup, "4", middleX-300, 5, "Verdana", 30)
...
function toggleSoundMedium(event)
...
function onPlayTouchMedium(event)
	if event.phase == "ended" then
...
function moveBoatSimMedium(event)
...
function addPlaneMedium(event)
	...
	gameScreenGroup:insert(planeFlying)
 	planeFlying.x = math.random(0,1200)
  	...
  	movePlaneMedium(planeFlying.x)
end	

function movePlaneMedium(compareX1)
...
--		print("this is pointRight")
	...
	planeFlying:play()
	planeMove = transition.to(planeFlying, {time = 1380, x = compareX2, y = display.contentHeight+10})
end

function tickingTimeMedium(event)
 theTime = theTime - 1
 timerNum.text = theTime 
end

function check4GameOverMedium()
	if lives<=0 then
--		print("gameOver")
		gameOverMedium("lives")	
	end
end

function gameOverMedium (event)
	print("gameOver on playMedium")
	if event == "lives" then
		lives=0
	end	
...
function removeObjectMedium(event)
--[[left this in for testing later	
	...
	lives = lives-1
	livesNum.text = lives
	check4GameOverMedium()
	end
end

function spawnBallMedium(event)
	if event.phase == "began" then
...
function scene:enterScene(event)
	print("enterScene playMedium")
	storyboard.purgeAll()
--timers
	addPlaneTimerMedium = timer.performWithDelay(750, addPlaneMedium, 37)
	endTimer = timer.performWithDelay(30000, gameOverMedium, 1)
	--timer for displayed timer
	countdownTimer=timer.performWithDelay(1000, tickingTimeMedium, 30)
...
-- Listeners
	destroyer:addEventListener("touch", moveBoatSimMedium)		
	Runtime:addEventListener("touch", spawnBallMedium)
	Runtime:addEventListener("collision", removeObjectMedium)
	Runtime:addEventListener("accelerometer", moveBoatMedium)
	musicIcon:addEventListener("touch", onPlayTouchMedium)
	sfxIcon:addEventListener("touch", toggleSoundMedium)
end

function scene:exitScene(event)
	timer.cancel(addPlaneTimerMedium)
	timer.cancel(endTimer)
	timer.cancel(countdownTimer)
	audio.stop()

-- Listeners		
	Runtime:removeEventListener("touch", spawnBallMedium)
	Runtime:removeEventListener("collision", removeObjectMedium)
	destroyer:removeEventListener("touch", moveBoatSimMedium)
	Runtime:removeEventListener("accelerometer", moveBoatMedium)
	musicIcon:removeEventListener("touch", onPlayTouchMedium)
	sfxIcon:removeEventListener("touch", toggleSoundMedium)
end
