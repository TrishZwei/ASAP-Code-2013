...
--Background
	...
	--oatsTimer = timer.performWithDelay(3000, fireOats, 1)
...
--Fires Oat Gun
	if granolaLives > 0 then
		...
		oats.myName="oats"
		physics.addBody(oats, "dynamic", {density = 1000000, friction = 1000000})
		--oats.collision=onCollision
		--oats:addEventListener("collision", oats)
		--oatsTimer = timer.performWithDelay(100, fireOats)
		oats.collision = onCollision
		oats:addEventListener("collision", oats)
		transition.to(oats, {time = 125, y = 400, onComplete = function(self) self.parent:remove(self); self=nil;end})
...
function onCollision(event)
	...
	if (event.phase == "began" and event.object1.myName == "granola" and event.object2.myName == "chocolate") then
 	...
 	elseif (event.phase == "began" and event.object1.myName == "oats" and event.object2.myName == "ourHero") then
